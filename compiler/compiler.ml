...
module Reader : READER = struct
  ...
  (* helping function*)
  let make_p car cdr= ScmPair (car, cdr);;
  let list_help sexprs sexpr = List.fold_right make_p sexprs sexpr;;
  ...
  and nt_vector str =
    ...
    let spaces_removing = star nt_whitespace in
    let nt_1= caten (word "#(") ( caten spaces_removing ( caten (caten inside spaces_removing) (char ')'))) in
    let nt_1= pack nt_1 (fun (_, (_,((y,_),_))) -> ScmVector(y)) in
    nt_1 str
    
  and nt_list str =
    let open_par = word "(" in
    let close_par = word ")" in
    let par = char ')' in
    let dot = char '.' in
    let nt_1' = caten nt_skip_star open_par in
    let nt_1 = caten nt_skip_star close_par in
    let nt_1 = caten nt_1 nt_skip_star in
    let nt_1 = pack nt_1 (fun (_) -> ScmNil) in
    let nt_2' = plus nt_sexpr in
    let nt_3 = pack par (fun (_) -> ScmNil) in
    let nt_5 = caten nt_sexpr par in
    let nt_5 = pack nt_5 (fun (sexpr, _)->sexpr) in
    let nt_4 = pack (caten dot nt_5) (fun (_, sexpr)->sexpr) in
    let nt_3 = disj nt_3 nt_4 in
    let nt_2' = pack (caten nt_2' nt_3) (fun (sexprs, sexpr)-> List.fold_right (make_p) sexprs sexpr) in
    let nt_2 = disj nt_1 nt_2' in
    let nt_1 = pack (caten nt_1' nt_2) (fun (_, sexpr) -> sexpr) in
    nt_1 str
...
module Tag_Parser : TAG_PARSER = struct
  ...
  let lambda_first func=  ScmPair (ScmSymbol "lambda", ScmPair (ScmNil, ScmPair (func, ScmNil)));;

  let lambda_second rest_of_cond= ScmPair (ScmSymbol "lambda", ScmPair (ScmNil, ScmPair (rest_of_cond, ScmNil)));;

  let cond_expr_func expr func remaining = 
    ScmPair (ScmSymbol "let", ScmPair (ScmPair(ScmPair (ScmSymbol "value", ScmPair (expr, ScmNil)), ScmPair (ScmPair
    (ScmSymbol "f", ScmPair (lambda_first func , ScmNil)),
      ScmPair (ScmPair (ScmSymbol "rest", ScmPair (lambda_second remaining, ScmNil)), ScmNil))),
          ScmPair (ScmPair (ScmSymbol "if", ScmPair (ScmSymbol "value", ScmPair (ScmPair (ScmPair (ScmSymbol "f", ScmNil),
            ScmPair (ScmSymbol "value", ScmNil)), ScmPair (ScmPair (ScmSymbol "rest", ScmNil), ScmNil)))), ScmNil)));;

  let rec macro_expand_cond_ribs ribs = 
    match ribs with 
    |ScmNil -> ScmVoid
    | ScmPair (ScmPair (ScmSymbol "else", expressions),ribs) -> ScmPair (ScmSymbol "begin", expressions)
    | ScmPair (ScmPair (expression, ScmPair (ScmSymbol "=>", ScmPair (func, ScmNil))), ribs) ->
      let remaining = macro_expand_cond_ribs ribs in
        cond_expr_func  expression func remaining 
    | ScmPair (ScmPair (pred, expressions), ribs) ->
      let remaining = macro_expand_cond_ribs ribs in
      ScmPair (ScmSymbol "if", ScmPair (pred, ScmPair (ScmPair (ScmSymbol "begin", expressions), ScmPair (remaining, ScmNil))))
    | _ -> raise (X_syntax "error macro_expand_cond_ribs come to me");;
  ...
  let is_list_of_unique_names =
    let rec run = function
      ...
      | _ -> false
    in run;;
  let rest_let  name_whatever sets_exprs=(ScmPair (ScmSymbol "let", ScmPair (name_whatever, sets_exprs)));;
  let help_whatever name ribs = ScmPair (ScmPair (name, ScmPair(ScmPair(ScmSymbol("quote"),ScmPair (ScmSymbol "'whatever", ScmNil)),ScmNil)), ribs);;
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
      let (names, values) = names_values ribs in 
      let name_whatever = List.fold_right(fun name ribs  -> help_whatever name ribs) names ScmNil in
      let sets = List.map2 (fun element1 element2 -> ScmPair (ScmSymbol "set!", ScmPair (element1, ScmPair (element2, ScmNil)))) names values in
      let sets_exprs = List.fold_right (fun car cdr -> ScmPair (car, cdr)) sets exprs in
        tag_parse (rest_let name_whatever sets_exprs)
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let rec run_address_enviroment pe params env =
    match pe with
    | ScmConst sexpr -> ScmConst' sexpr  
    | ScmVarGet (Var str_) -> ScmVarGet' (tag_lexical_address_for_var str_ params env)     
    | ScmIf (test, then_ ,rest_) -> ScmIf' (run_address_enviroment test params env, run_address_enviroment then_ params env, run_address_enviroment rest_ params env)  
    | ScmSeq expressions -> ScmSeq' (List.map (fun expression-> run_address_enviroment expression params env) expressions)     
    | ScmOr expressions -> ScmOr' (List.map (fun expression-> run_address_enviroment expression params env) expressions)     
    | ScmVarSet(Var v_, expression) -> ScmVarSet' (tag_lexical_address_for_var v_ params env,run_address_enviroment expression params env)     
    | ScmVarDef(Var v_, expression) -> ScmVarDef' (tag_lexical_address_for_var v_ params env,run_address_enviroment expression params env)      
    | ScmLambda (params',type_, expression) -> 
        let lambda_check  type_  = 
            match type_ with
              | Opt opt  -> ScmLambda' (params' , Opt opt, run_address_enviroment expression (params' @ [opt]) (params :: env))
              | Simple -> ScmLambda' (params' , Simple, run_address_enviroment expression params' (params :: env)) in
              lambda_check  type_
    | ScmApplic (proc, args) ->  ScmApplic' (run_address_enviroment proc params env, List.map (fun arg -> run_address_enviroment arg params env) args, Non_Tail_Call);;


  let annotate_lexical_address pe =
    run_address_enviroment pe [] [];;

  (* run this second *)
    let rec fine_tail_pos tail_pos = function
      | (ScmConst' _) as original -> original
      | (ScmVarGet' _) as original -> original
      | ScmSeq' [] -> ScmSeq'([])
      | ScmSeq' (expression :: expressions) -> ScmSeq' (run tail_pos expression expressions)
      | ScmIf' (test, then_, rest_) -> ScmIf'(fine_tail_pos false test, fine_tail_pos tail_pos then_, fine_tail_pos tail_pos rest_)
      | ScmOr' [] -> ScmOr'([])
      | ScmOr' (expression :: expressions) -> ScmOr' (run tail_pos expression expressions)
      | ScmVarSet' (var', expression') -> ScmVarSet'(var', fine_tail_pos false expression')
      | ScmVarDef' (var', expression') -> ScmVarDef'(var', fine_tail_pos false expression')
      | (ScmBox' _) as expr' -> expr'
      | (ScmBoxGet' _) as expression' -> expression'
      | ScmBoxSet' (var', expression') -> ScmBoxSet'(var', fine_tail_pos false expression')
      | ScmLambda' (parameters, Simple, expression) -> ScmLambda'(parameters, Simple, fine_tail_pos true expression)
      | ScmLambda' (parameters, Opt opt, expression) -> ScmLambda'(parameters, Opt opt, fine_tail_pos true expression)
      | ScmApplic' (proc, arguments, app_kind) -> applic_test tail_pos proc arguments app_kind
    and run tail_pos expression = function
      | [] -> [fine_tail_pos tail_pos expression]
      | expression' :: exprs -> (fine_tail_pos false expression) :: (run tail_pos expression' exprs)
    and applic_test tail_pos proc arguments app_kind = 
       let help tail_pos =
          match tail_pos with
          | true -> ScmApplic' (fine_tail_pos false proc, List.map (fun arguments -> fine_tail_pos false arguments) arguments, Tail_Call) 
          | false ->  ScmApplic' (fine_tail_pos false proc,  List.map (fun arguments -> fine_tail_pos false arguments) arguments, Non_Tail_Call) in
        help tail_pos;;
  
  let annotate_tail_calls pe =  
    fine_tail_pos false pe;;
...
module Code_Generation : CODE_GENERATION = struct
  ...
  let asm_of_constants_table =
    ...
    let collect_free_vars =
      let rec go_find_free = function
        | ScmConst' _ -> []
        | ScmVarGet' (Var' (v_, Free)) -> [v_]
        | ScmVarGet' _ -> []
        | ScmSeq' expressions' -> go_find_free2 expressions'
        | ScmOr' expressions' -> go_find_free2 expressions'
        | ScmBox' (Var' (v_, Free)) -> [v_]
        | ScmBox' _ -> []
        | ScmIf' (test, then_, rest_) -> (go_find_free test) @ (go_find_free then_) @ (go_find_free rest_)
        | ScmBoxSet' (Var' (v_, Free), expression') -> [v_] @ (go_find_free expression')
        | ScmBoxSet' (_, expression') -> go_find_free expression'
        | ScmVarSet' (Var' (v_, Free), expression') -> [v_] @ (go_find_free expression')
        | ScmVarSet' (_, expression') -> go_find_free expression'
        | ScmVarDef' (Var' (v_, Free), expression') -> [v_] @ (go_find_free expression')
        | ScmVarDef' (_, expression') -> go_find_free expression'
        | ScmBoxGet' (Var' (v_, Free)) -> [v_]
        | ScmBoxGet' _ -> []
        | ScmLambda' (_, _, expression') -> go_find_free expression'
        | ScmApplic' (expression', expressions', _) -> (go_find_free expression') @  (go_find_free2 expressions') 
      and go_find_free2 expressions' =  List.fold_left  (fun vars expression' -> vars @ (go_find_free expression')) [] expressions'  
      in fun expressions' ->  let free_vars_in_code = go_find_free2 expressions' in
      remove_duplicates (primitives_var @ free_vars_in_code);;
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmIf' (test, then_, rest_) ->
        let label_rest =  make_if_else ()in
        let label_end = make_if_end () in
        let result_of_test =(run params env test) in
        let result_of_then_ = (run params env then_) in
        let result_of_rest_ = (run params env rest_) in
        result_of_test ^
        (Printf.sprintf "\t cmp rax, sob_boolean_false\n\tje %s\n"label_rest)^
        result_of_then_^
        (Printf.sprintf "\tjmp %s\n\t%s:" label_end label_rest)^
        result_of_rest_^
        (Printf.sprintf  "%s:\n" label_end)
      | ScmSeq' exprs' ->
         String.concat "\n"
      ...
      | ScmBoxGet' var' ->
        let result = (run params env (ScmVarGet' var')) in
        result
         ^ "\tmov rax, qword [rax]\n"
      | ScmBoxSet' (var', expr') ->
      ...
      | ScmLambda' (params', Opt opt, body) ->
        ...
        let label_loop_parameters_last = make_lambda_opt_loop_params_end ()in
        let label_loop_enviroment = make_lambda_opt_loop_env () in
        let label_loop_enviroment_end = make_lambda_opt_loop_env_end () in
        ...
        let label_arity_else = make_lambda_opt_arity_more () in
        ...
        ^(Printf.sprintf "\tmov rdi, 8 * %d\t; extended env\n" (env + 1))
        ^"\tcall malloc\n\tmov rdi, ENV\n\tmov rsi, 0\n\tmov rdx, 1\n"
        ^(Printf.sprintf "%s:\t; ext_env[i + 1] <-- env[i] enviroment extending \n" label_loop_enviroment)
        ^(Printf.sprintf "\tcmp rsi, %d\n\tje %s\n" (env)  label_loop_enviroment_end)
        ^"\tmov rcx, qword [rdi + 8 * rsi]\n\tmov qword [rax + 8 * rdx], rcx\n\tinc rsi\n\tinc rdx\n"
        ^(Printf.sprintf "\tjmp %s\n%s:\n" label_loop_enviroment label_loop_enviroment_end)
        ^"\tpop rbx\n\tmov rsi, 0\n"
        ^(Printf.sprintf "%s:\t; copy params\n\tcmp rsi, %d\n\tje %s\n" label_loop_parameters params label_loop_parameters_last)
        ^"\tmov rdx, qword [rbp + 8 * rsi + 8 * 4]\n"
        ^"\tmov qword [rbx + 8 * rsi], rdx\n"
        ^"\tinc rsi\n"
        ^(Printf.sprintf "\tjmp %s\n%s:\n" label_loop_parameters label_loop_parameters_last)
        ^"\tmov qword [rax], rbx\t \n\tmov rbx, rax\n\tpop rax\n\tmov byte [rax], T_closure\n\tmov SOB_CLOSURE_ENV(rax), rbx\n"
        ^(Printf.sprintf "\tmov SOB_CLOSURE_CODE(rax), %s\n" label_code)
